      ...
      <div className="w-[800px] bg-white border p-10 mb-8 shadow-sm box-border flex flex-col justify-between min-h-[1050px]">
        <div>
          ...
          <div className="grid grid-cols-2 gap-4 text-xs border p-4 rounded-xl bg-slate-50/50">
            <div><span className="text-slate-400">नाम (Full Name):</span> <p className="font-bold text-slate-900 text-sm">{student?.name}</p></div>
            <div><span className="text-slate-400">पिता का नाम (Father Name):</span> <p className="font-bold text-slate-800">{student?.father_name || "N/A"}</p></div>
            <div><span className="text-slate-400">अनुक्रमांक (Roll No):</span> <p className="font-mono font-bold text-slate-800">{student?.roll_no}</p></div>
            <div><span className="text-slate-400">पंजीकरण संख्या (Reg No):</span> <p className="font-mono font-bold text-slate-800">{student?.reg_no}</p></div>
            <div><span className="text-slate-400">संस्थान / यूनिवर्सिटी:</span> <p className="font-bold text-slate-800">{student?.college} ({student?.university})</p></div>
            <div><span className="text-slate-400">संकाय (Faculty / Stream):</span> <p className="font-bold text-slate-800">{student?.faculty} ({student?.year})</p></div>
          </div>
